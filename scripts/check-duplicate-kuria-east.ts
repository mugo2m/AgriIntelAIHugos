...
async function main() {
  console.log("");
  console.log("Checking Migori / Kuria East SubCounty records...");
  console.log("");
  ...
  const records = await prisma.subCounty.findMany({
    where: {
      ...
      name: {
        in: ["Kuria East", "Kuria East Sub County"],
        mode: "insensitive",
      },
    },
    include: {
  ...
  if (records.length > 1) {
    console.log("");
    console.log("Duplicate Kuria East records detected.");
  } else {
    console.log("");
    console.log("No duplicate Kuria East records detected.");
  }
}
...
main()
  .catch((error) => {
    console.error("");
    console.error("Failed to inspect Kuria East records.");
    console.error(error);
    process.exit(1);
  })
  .finally(async () => {
