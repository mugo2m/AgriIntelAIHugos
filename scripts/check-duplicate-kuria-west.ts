...
async function main() {
  console.log("");
  console.log("Checking Migori / Kuria West SubCounty records...");
  console.log("");
  ...
  const records = await prisma.subCounty.findMany({
    where: {
      ...
      name: {
        in: ["Kuria West", "Kuria West Sub County"],
        mode: "insensitive",
      },
    },
    include: {
  ...
  if (records.length > 1) {
    console.log("");
    console.log("Duplicate Kuria West records detected.");
  } else {
    console.log("");
    console.log("No duplicate Kuria West records detected.");
  }
}
...
main()
  .catch((error) => {
    console.error("");
    console.error("Failed to inspect Kuria West records.");
    console.error(error);
    process.exit(1);
  })
  .finally(async () => {
