// lib/actions/general.action.ts – UPDATED
// VERSION: 3.3 – Full dairy agent data in session summaries + structuredList handling
"use server";
...
function buildFarmerQaHistory(messages: any[]) {
  ...
  console.log(
    `🌾 Built ${pairs.length} Q&A pairs from farmer session (${pairs.filter((p) => p.isFinancial).length} financial, ${pairs.filter((p) => p.isSoilTest).length} soil test, ${pairs.filter((p) => p.isDamage).length} damage reports)`,
  );
...
function formatCurrencyForCountry(
  ...
  const currency =
    COUNTRY_CURRENCY_MAP[country] || COUNTRY_CURRENCY_MAP.kenya;
...
function formatCurrencyForSpeech(
  amount: number,
  country: string = "kenya",
): string {
  const currency =
...
const defaultYields: Record<string, number> = {
  ...
  canavalia: 2000,
  "sunn hemp": 2000,
  "crotalaria paulina": 2000,
  cassava: 8000,
  "sweet potatoes": 7000,
  "irish potatoes": 10000,
  yams: 12000,
  taro: 10000,
  ginger: 8000,
  turmeric: 6000,
  ...
  cucumbers: 10000,
  "pumpkin leaves": 8000,
  "sweet potato leaves": 8000,
  "ethiopian kale": 8000,
  "jute mallow": 6000,
  "spider plant": 6000,
  "african nightshade": 5000,
  amaranth: 4000,
  arugula: 5000,
  asparagus: 3000,
  artichoke: 5000,
  ...
  "buffel grass": 6000,
  "guinea grass": 8000,
  "italian ryegrass": 8000,
  "napier grass": 20000,
  "napier hybrid": 25000,
  "orchard grass": 8000,
  "rhodes grass": 8000,
  "timothy grass": 8000,
  "forage sorghum": 15000,
  leucaena: 8000,
  calliandra: 8000,
  sesbania: 8000,
  cenchrus: 6000,
...
const defaultPrices: Record<string, number> = {
  ...
  cucumbers: 30,
  "pumpkin leaves": 20,
  "sweet potato leaves": 20,
  "ethiopian kale": 20,
  "jute mallow": 20,
  "spider plant": 20,
  "african nightshade": 30,
  amaranth: 20,
  arugula: 30,
  asparagus: 100,
  artichoke: 80,
...
function estimateCosts(
  ...
  let baseSeed = 2000;
  let baseFertilizer = 8000;
  let baseLabour = 15000;
  let baseTransport = 2000;
  let baseBagCost = 1000;

  switch (category) {
    case "cereal":
      baseSeed = 2500;
      baseFertilizer = 10000;
      baseLabour = 18000;
      baseTransport = 2500;
      baseBagCost = 1500;
      break;
...
function calculateGrossMargin(session: any) {
  ...
  return {
    ...
    recommendation: getFinancialRecommendation(
      session.managementLevel,
      result.low,
      result.medium,
      result.high,
      country,
...
function getFinancialRecommendation(
  level: string,
  low: any,
  medium: any,
  high: any,
...
export async function generateFarmerSessionSummary(params: {
  ...
  try {
    ...
    if (isCrop && session.soilTest) {
      const ageStatus = soilTestInterpreter.getTestAgeStatus(
        session.soilTest.testAge || 0,
      );
...
`;
          ...
          const dairyDetails = details.length
            ? details.join("\n")
...
`;
    ...
    if (!summary) {
      ...
      if (isCrop) {
        ...
        fallbackFinancialAdvice =
          grossMargin?.recommendation ||
          "Track all input costs (seeds, fertilizer, labour) to calculate your actual profit margins. Farming is a BUSINESS!";

        fallbackSoilTestAdvice = soilTestSummary
          ? `Your soil test from ${soilTestSummary.testDate} shows ${soilTestSummary.phRating} pH and ${soilTestSummary.phosphorusRating} phosphorus.`
          : "Consider doing a soil test for precise fertilizer recommendations - it can save you up to 30% on fertilizer costs!";

        fallbackDamageAdvice = session.plantsDamaged
          ? `You reported ${session.plantsDamaged} plants damaged beyond recovery. Consider reviewing your pest and disease management strategies.`
          : "";

        fallbackExtensionAdvice = session.extensionInputs
          ? `You provided extension officer recommendations: ${session.extensionInputs.plantingFertilizerType} at ${session.extensionInputs.plantingFertilizerQuantity} kg/acre.`
          : "";

        fallbackTopics = session?.crops || ["general farming"];
        fallbackNextSteps =
          "Continue asking questions about your specific crops to maximize profitability.";
      } else if (isPoultry) {
        ...
        fallbackFinancialAdvice =
          "Track all costs (feed, vaccination, medication, housing) and revenue (egg/meat sales) to calculate your profit per bird. Every shilling saved is a shilling earned.";

        fallbackSoilTestAdvice = "";

        fallbackDamageAdvice =
          session.poultry?.mortalityCount > 0
            ? `You reported ${session.poultry.mortalityCount} bird losses. Consider a post-mortem examination to identify the cause.`
            : "";

        fallbackExtensionAdvice = "";
        fallbackTopics = ["poultry farming"];
        fallbackNextSteps =
          "Continue asking about specific poultry health, nutrition, and management issues.";
      } else if (isDairy) {
        const dairy = session.dairy || {};
        ...
        const category = dairy.cowCategory
          ? ` - ${dairy.cowCategory}`
          : "";
        const disease = dairy.diseaseSelect
          ? ` for ${dairy.diseaseSelect}`
          : "";
        ...
        fallbackFinancialAdvice =
          "Analyze your milk production per cow, feed costs, and veterinary expenses to calculate daily profit. Consider group purchasing of feed and medicines to reduce costs.";

        fallbackSoilTestAdvice = "";

        fallbackDamageAdvice =
          dairy.mortalityCount > 0
            ? `You reported ${dairy.mortalityCount} cow deaths. Review your herd health management immediately.`
            : "";

        fallbackExtensionAdvice = "";
        fallbackTopics = ["dairy farming"];
        fallbackNextSteps =
          "Continue asking about dairy health, nutrition, reproduction, and business management.";
      }
...
// 🌾 UPDATED: Get farmer session by ID
// Explicitly preserves structuredList from the session.
// Supports both:
// 1. Current sessions: data.structuredList
// 2. Older sessions: data.feedResult.structuredList
export async function getFarmerSessionById(
  id: string,
  language: string = "en",
): Promise<any> {
    ...
    const session = await CacheManager.getOrSet(
      ...
      async () => {
        ...
        if (
          ...
          console.log(
            "📊 Adding default gross margin for old crop session (no real data)",
          );

          data.grossMarginAnalysis = calculateGrossMargin(data);
        } else if (data && data.grossMarginAnalysis) {
          console.log(
            "✅ Using existing gross margin data from farmer's actual inputs",
          );
        }
        ...
        // ✅ STRUCTURED LIST FIX
        // ============================================================
        // Current farmer_sessions documents store structuredList at
        // the top level.
        //
        // Older documents may have structuredList nested inside
        // feedResult. Preserve the existing top-level value when
        // available, otherwise recover it from feedResult.
        //
        // We deliberately do NOT write this back to Firestore.
        // This is only normalization of the object returned to callers.
        // ============================================================

        if (data) {
          if (
            !Array.isArray(data.structuredList) &&
            Array.isArray(data.feedResult?.structuredList)
          ) {
            data.structuredList = data.feedResult.structuredList;

            console.log(
              "📋 structuredList restored from feedResult for session:",
              id,
            );
          } else if (Array.isArray(data.structuredList)) {
            console.log(
              "📋 Using existing top-level structuredList for session:",
              id,
            );
          } else {
            // Keep a predictable shape for consumers of this function.
            data.structuredList = [];
          }
        }
...
export async function getFarmerSessionsByUserId(
  userId: string,
): Promise<any[]> {
  if (!userId) {
  ...
  try {
    ...
    console.log(
      `🔍 [DEBUG] Total sessions found in DB: ${sessionsWithoutOrder.size}`,
    );
    ...
    console.log(
      `🔍 [DEBUG] After manual sort: ${allSessions.length} sessions`,
    );
    ...
    return await CacheManager.getOrSet(
      ...
      async () => {
        console.log(
          `🔍 [DEBUG] Cache miss - returning ${allSessions.length} sessions`,
        );
...
export async function saveFarmerQuery(params: {
  ...
  try {
    ...
    await db.collection("farmer_sessions").doc(sessionId).update({
      ...
      financialQueryCount: isFinancial
        ? FieldValue.increment(1)
        : FieldValue.increment(0),
      soilTestQueryCount: isSoilTest
        ? FieldValue.increment(1)
        : FieldValue.increment(0),
      damageReportCount: isDamage
        ? FieldValue.increment(1)
        : FieldValue.increment(0),
  ...
  try {
    ...
    return {
      ...
      fertilizerRecommendations:
        session.soilTest.fertilizerRecommendations,
    };
  } catch (error: any) {
...
export async function getInterviewById(id: string): Promise<any> {
  console.warn(
    "⚠️ getInterviewById is deprecated. Use getFarmerSessionById instead.",
  );
...
export async function getInterviewsByUserId(
  ...
  console.warn(
    "⚠️ getInterviewsByUserId is deprecated. Use getFarmerSessionsByUserId instead.",
  );
...
export async function getFeedbackByInterviewId(params: {
  ...
  console.log(
    "📝 getFeedbackByInterviewId called (returning null)",
  );
...
export async function createFeedback(params: any): Promise<any> {
  return {
    success: false,
    error:
      "Feedback creation is deprecated. Use generateFarmerSessionSummary instead.",
  };
}
